const ProfitSharingMock = artifacts.require("ProfitSharingMock.sol");
const StandardToken = artifacts.require("MiniMeToken.sol");
...
contract('ProfitSharingMock', function (accounts) {
  ...
  let token;
  beforeEach(async () => {
    const sale = await ProfitSharingMock.new(accounts[0], 860) // 140 extra tokens are 14% extra at sale end
  ...
  it("tests profit sharing functionality", async () => {
    ...
    await token.approve(accounts[2], 300);
    await token.approve(accounts[4], 400);
    ...
    await XREDCoin.generateTokens(accounts[3], 800);
